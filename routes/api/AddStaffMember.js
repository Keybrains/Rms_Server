...
router.put("/staffmember/:id", async (req, res) => {
  try {
    ...
    let result = await AddStaffMember.findByIdAndUpdate(
      req.params.id,
      req.body
    );
    res.json({
...
router.post("/login", async (req, res) => {
  try {
    const user = await AddStaffMember.findOne({
      staffmember_email: req.body.staffmember_email,
    });
    if (!user) {
      return res.json({ statusCode: 403, message: "User doesn't exist" });
    }
    const isMatch = await AddStaffMember.findOne({
      staffmember_password: req.body.staffmember_password,
    });
    if (!isMatch) {
